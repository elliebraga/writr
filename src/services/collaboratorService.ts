...
export const collaboratorService = {
  ...
  // Adicionar/Convidar um novo colaborador por e-mail (com resiliência total para e-mail, nome da obra e status no Supabase)
  async addCollaborator(
    ...
    await ensureParentBookExists(safeBookId, bookName || "Obra");
    ...
    const newCollaborator: BookCollaborator = {
      ...
      role: role,
      status: "pending",
      book_name: bookName || "Obra Compartilhada",
      created_at: new Date().toISOString(),
    };
    ...
    // 2. Persistir no Supabase com resiliência de colunas (salvando book_name se a coluna existir)
    try {
      ...
      // Se falhar por conta de colunas não existentes no Supabase
      if (error) {
