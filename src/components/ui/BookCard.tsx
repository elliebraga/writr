...
const BookCard = React.forwardRef<HTMLDivElement, BookCardProps>(
  (
    ...
    const baseStyles = cn(
      "group relative flex flex-col w-full bg-white transition-all duration-200 ease-in-out cursor-pointer select-none overflow-hidden font-sans",
      "rounded-card border",
      active
        ? "border-2 border-neutral-900 shadow-xs p-[15px]"
        : "border-neutral-200 hover:border-neutral-400 p-4"
    );
    ...
    if (variant === "add") {
      return (
        <div
          ref={ref}
          onClick={onClick}
          className={cn(baseStyles, "items-center justify-center min-h-[320px] border-2 border-dashed border-neutral-300 hover:border-neutral-900", className)}
          {...props}
        >
          <svg
